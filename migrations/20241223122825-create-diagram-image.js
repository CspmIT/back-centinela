...
module.exports = {
	async up(queryInterface, Sequelize) {
		await queryInterface.createTable('DiagramImages', {
			...
			statusTopic: { type: Sequelize.BOOLEAN },
			topic: { type: Sequelize.STRING },
			showValue: { type: Sequelize.BOOLEAN },
			valuePosition: { type: Sequelize.STRING },
			sizeTextValue: { type: Sequelize.INTEGER },
			colorTextValue: { type: Sequelize.STRING },
			backgroundTextValue: { type: Sequelize.STRING },
			createdAt: {
				allowNull: false,
				type: Sequelize.DATE,
			},
